...
export const updateBatch = async (req, res) => {
  try {
    ...
    // 🔥 ДОБАВЛЕНО: 1. Вытаскиваем старые версии UPS из базы (только поле image)
    const itemIds = updatedItems.map((i) => i._id).filter(Boolean);
    const existingItems = await UpsModel.find(
...
export const deleteBatch = async (req, res) => {
  ...
  try {
    ...
    upsList.forEach((u) => {
      if (u.__localId) {
        localIdsToReturn.push(u.__localId.toString());
      }
      if (u.image) {
        imagesToDeletePromises.push(deleteOldCloudinaryImage(u.image));
      }
    });
