...
//   try {
//     const now = new Date(); // 1. Находим клеммные блоки для удаления
...
//         success: true,
//         message:
//           "Не найдены клеммные блоки для мягкого удаления (возможно, уже удалены).",
//         successDeletedDocIds: [],
//         failedDeletedDocIds: ids.map((id) => id.toString()),
//       });
//     } // --- 2. КАСКАДНЫЙ ПОИСК ДОЧЕРНИХ ЭЛЕМЕНТОВ (Сигналы) --- // 2a. Signal (Сигнал)
...
//       .session(session)
//       .select("__localId");
//     signalLocalIDs = signalsToUpdate.map((doc) => doc.__localId); // --- 3. МЯГКОЕ УДАЛЕНИЕ ДОЧЕРНИХ ЭЛЕМЕНТОВ (Сигналы) --- // 3a. Signal (самый нижний уровень)
//     if (signalLocalIDs.length > 0) {
//       await SignalModel.updateMany(
//         { __localId: { $in: signalLocalIDs } },
//         {
...
//         { session: session }
//       );
//     } // --- 4. МЯГКОЕ УДАЛЕНИЕ САМИХ КЛЕММНЫХ БЛОКОВ (TerminalBlock) ---

//     const result = await TerminalBlockModel.updateMany(
//       { __localId: { $in: terminalBlockLocalIDs } },
//       {
//         $set: {
//           signals: [], // !!! Очищаем массив ссылок на Signal (даже если он уже пуст)
//           isPendingDeletion: true,
//           deletedAt: now,
//           updatedAt: now,
//         },
//       },
//       { session }
//     ); // --- 5. ОЧИСТКА РОДИТЕЛЬСКОГО ШКАФА (EnclosureItem) --- // Группируем удаленные клеммные блоки по родительскому enclosureItem
...
//         updateOne: {
//           // Ищем EnclosureItem по его __localId
//           filter: { __localId: enclosureLocalId },
//           update: {
//             // Удаляем ссылки на удаленные клеммные блоки из массива terminalBlocks
//             $pullAll: {
//               terminalBlocks: enclosureTerminalMap[enclosureLocalId],
...
//     const successDeletedDocIds = terminalBlocksMongoDB.map((t) =>
//       t._id.toHexString()
//     );
...
//       return res.status(200).json({
//         success: true,
//         message: `Успешно помечено на удаление ${result.modifiedCount} клеммных блоков и их дочерних элементов.`,
//         successDeletedDocIds,
//       });
//     }
//   } catch (error) {
