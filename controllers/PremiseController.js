      ...
      updateOperations.push({
        updateOne: {
          ...
          update: {
            ...
            $set: { updatedAt: new Date() }, // Исправлено (нужен $set)
          },
        },
      });
    }
...
export const updateBatch = async (req, res) => {
  ...
  try {
    ...
    const oldDocs = await PremiseModel.find({ _id: { $in: docServerIds } })
      .select("_id __localId section")
      .lean(); // lean()

    const oldDocsMap = oldDocs.reduce((map, doc) => {
    ...
    const pushToSections = {};

    for (const doc of updatedDocs) {
      if (!doc._id || !mongoose.Types.ObjectId.isValid(doc._id)) continue;
      ...
      const premiseLocalId = oldPremise.__localId;
    ...
    if (updateOperations.length > 0) {
      const updatePremiseResult = await PremiseModel.bulkWrite(
        updateOperations,
        {
          session,
          ordered: false,
      ...
      // (Обработка writeErrors пропущена для краткости, она не критична для WriteConflict)
    }
    ...
    if (pullOps.length > 0) await SectionModel.bulkWrite(pullOps, { session });
    ...
    if (pushOps.length > 0) await SectionModel.bulkWrite(pushOps, { session });
...
// --- DELETE BATCH (ИСПРАВЛЕНО ДЛЯ WRITE CONFLICT) ---
export const deleteBatch = async (req, res) => {
  ...
  // 1. ЧТЕНИЕ (ВНЕ ТРАНЗАКЦИИ)
  // Находим удаляемые помещения и всех их детей, чтобы собрать ID
  ...
  const initialValidIdStrings = Array.from(validIdMap.keys());

  if (validIds.length === 0) {
    return res
      .status(400)
      .json({ message: "Не предоставлено ни одного корректного ID." });
  }

  try {
    // 1.1 Находим сами помещения
    ...
    // 1.2 Находим всех детей (EnclosureItems, TerminalBlocks, Signals)
    // Делаем это ДО транзакции, чтобы не держать её открытой во время поиска
    const enclosureItems = await EnclosureItemModel.find({
    ...
    const enclosureItemLocalIDs = enclosureItems.map((doc) => doc.__localId);
    ...
    if (enclosureItemLocalIDs.length > 0) {
      const terminalBlocks = await TerminalBlockModel.find({
        ...
        .select("__localId")
        .lean();
      terminalBlockLocalIDs = terminalBlocks.map((doc) => doc.__localId);
    }
    ...
    if (terminalBlockLocalIDs.length > 0) {
      const signals = await SignalModel.find({
        ...
        .select("__localId")
        .lean();
      signalLocalIDs = signals.map((doc) => doc.__localId);
    }
    ...
    // 2.1 Массовое обновление (Soft Delete) детей
    if (signalLocalIDs.length > 0) {
      await SignalModel.updateMany(
        { __localId: { $in: signalLocalIDs } },
        { $set: { isPendingDeletion: true, updatedAt: now } },
        { session }
      );
    }
    if (terminalBlockLocalIDs.length > 0) {
      await TerminalBlockModel.updateMany(
        ...
        { $set: { signals: [], isPendingDeletion: true, updatedAt: now } },
        { session }
      );
    }
    if (enclosureItemLocalIDs.length > 0) {
      await EnclosureItemModel.updateMany(
        ...
        {
          $set: { terminalBlocks: [], isPendingDeletion: true, updatedAt: now },
        },
        { session }
      );
    }

    // 2.2 Удаление из родительских секций
    const sectionPremisesMap = {};
    premisesMongoDB.forEach((premise) => {
      // premise.section может быть объектом (если lean) или строкой
    ...
    // Формируем ответ
    const successfullyMarkedIds = premisesMongoDB.map((p) => p._id.toString());
    const alreadyDeletedIds = initialValidIdStrings.filter(
      (idStr) => !successfullyMarkedIds.includes(idStr)
    );

    return res.status(200).json({
      success: true,
      successDeletedDocIds: [...successfullyMarkedIds, ...alreadyDeletedIds],
      failedDeletedDocIds: [],
    });
  } catch (error) {
    if (session.inTransaction()) await session.abortTransaction();
