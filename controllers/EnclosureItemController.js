...
//   try {
//     const now = new Date(); // 1. Находим шкафы для удаления и их родительские помещения

//     const enclosureItemsMongoDB = await EnclosureItemModel.find({
//       _id: { $in: validIds }, // Ищем по Server ID
//       isPendingDeletion: false,
//     })
//       .select("__localId premise _id") // Нужно __localId, premise, __serverId
//       .session(session);
...
//         success: true,
//         message:
//           "Не найдены шкафы для мягкого удаления (возможно, уже удалены).",
//         successDeletedDocIds: [],
//         failedDeletedDocIds: ids.map((id) => id.toString()),
//       });
//     } // --- 2. КАСКАДНЫЙ ПОИСК ДОЧЕРНИХ ЭЛЕМЕНТОВ (ТОЛЬКО ПОИСК ID) --- // 2a. TerminalBlock (Клеммная колодка)
...
//     terminalBlockLocalIDs = terminalBlocksToUpdate.map((doc) => doc.__localId); // 2b. Signal (Сигнал)
//     let signalLocalIDs = [];
//     if (terminalBlockLocalIDs.length > 0) {
//       const signalsToUpdate = await SignalModel.find({
//         terminalBlock: { $in: terminalBlockLocalIDs },
...
//         .select("__localId");
//       signalLocalIDs = signalsToUpdate.map((doc) => doc.__localId);
//     } // --- 3. МЯГКОЕ УДАЛЕНИЕ ДОЧЕРНИХ ЭЛЕМЕНТОВ (СНИЗУ ВВЕРХ) --- // 3a. Signal (самый нижний уровень)
...
//         { session: session }
//       );
//     } // 3b. TerminalBlock (мягкое удаление и очистка массива signals)
...
//         {
//           $set: {
//             signals: [], // !!! Очистка массива ссылок на Signal
//             isPendingDeletion: true,
//             deletedAt: now,
//             updatedAt: now,
//           },
...
//         { session: session }
//       );
//     } // --- 4. МЯГКОЕ УДАЛЕНИЕ САМИХ ШКАФОВ (EnclosureItem) ---

//     const result = await EnclosureItemModel.updateMany(
//       { __localId: { $in: enclosureItemLocalIDs } },
//       {
//         $set: {
//           terminalBlocks: [], // Очищаем массив ссылок на TerminalBlock
//           isPendingDeletion: true,
//           deletedAt: now,
//           updatedAt: now,
//         },
//       },
//       { session }
//     ); // --- 5. ОЧИСТКА РОДИТЕЛЬСКОГО ПОМЕЩЕНИЯ (Premise) --- // Группируем удаленные шкафы по родительскому premise.__localId
...
//       (premiseId) => ({
//         updateOne: {
//           // Ищем Premise по его __localId
//           filter: { __localId: new mongoose.Types.ObjectId(premiseId) },
//           update: {
//             // Удаляем ссылки на удаленные шкафы из массива enclosureItems
//             $pullAll: { enclosureItems: premiseEnclosureMap[premiseId] },
//             $set: { updatedAt: now },
//           },
//         },
...
//     await session.commitTransaction();
//     isCommitted = true; // 6. Формирование ответа

//     const successDeletedDocIds = enclosureItemsMongoDB.map((e) =>
//       e._id.toHexString()
//     );
...
//       return res.status(200).json({
//         success: true,
//         message: `Успешно помечено на удаление ${result.modifiedCount} шкафов и их дочерних элементов.`,
//         successDeletedDocIds,
//       });
//     }
//   } catch (error) {
